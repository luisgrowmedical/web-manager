...
class SiteController {
    ...
    public function view($id) {
        ...
        $stmt = $this->db->prepare("SELECT * FROM site_metrics WHERE site_id = ? ORDER BY sync_date DESC LIMIT 1");
        $stmt->execute([$id]);
        $metrics = $stmt->fetch();
        ...
        if ($http_code === 200) {
            ...
            if ($site_data && isset($site_data['site_url'])) {
                ...
                $stmt = $this->db->prepare("
                    ...
                    (site_id, pages_count, posts_count, drafts_count, active_theme, themes_count, active_plugins_count, total_plugins_count, site_weight, images_count) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ");
                $stmt->execute([
                    $id,
